...
js264.prototype.decodeSocketHeader = function( data ) {
	...
	if( 
		...
		decoder = new Worker('../decoder_wasm/decoder_worker_265_noQSBP.js');
		...
		decoder = new Worker('../decoder_wasm/decoder_worker_264_noQSBP.js');
...
//======================================
    ...
    decoder.addEventListener('message', function(e) {
        ...
        case 'pictureReady':
			...
			++pictureCount;
			updateFrameRate();  // 更新编码器，抓屏名称，帧率等信息
            break;
        case 'decoderReady':
...
//======================================	
    ...
    if( this.sequenceStarted2 ) {
		var copy = buf.slice(jsmpegHeaderSize);
