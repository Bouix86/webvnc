...
self.importScripts('libffmpeg_264.js');
...
function onMessage(e) {
    ...
    var buf = new Uint8Array(message.data).subarray(jsmpegHeaderSize);
    var size = buf.length;
        ...
        case 'queueInput':
            ...
            if (dataLength > CHUNK_SIZE) {
                do {
                    ...
                    var data = buf.subarray(offset, offset + len);
                    // var typedArray = new Uint8Array(data); // todo: 这里是否需要new一下, 如果不new, 可能导致堆出问题
                    dataLength -= len;
                    offset += len;
...
function initFinish() {
    ...
    var videoSize = 0;
    var videoCallback = Module.addFunction(function (addr_y, addr_u, addr_v, stride_y, stride_u, stride_v, width, height, pts, qp_cur, size_pkt) {
        // console.log("[%d]In video callback, size = %d * %d, stride_y = %d, stride_u = %d, stride_v = %d, pts = %d, qp_cur = %d, size_pkt = %d, psnr_cur = %d", ++videoSize, width, height, stride_y, stride_u, stride_v, pts, qp_cur, size_pkt, psnr_cur)
